...
def score(results: dict) -> int:
    ...
    legacy_tls_versions = {"tls.1.0", "tls.1.1"}
    legacy_tls_count = sum(1 for v in tls_versions if v in legacy_tls_versions)
    if legacy_tls_count > 0:
        base_score = max(5, base_score - legacy_tls_count * 5)
